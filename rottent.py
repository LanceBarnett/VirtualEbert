...
"""
Slightly modified code from https://github.com/zachwill/rottentomatoes
(Added some status checks)
...
class RT(object):
    ...
    def _load_json_from_url(self, url, status):
      """
      A wrapper around the api call. The response might be gzipped,
      which this will abstract away.
      Returns a json-decoded object.

      Added status argument - Lance
      """
      url_opened = urlopen(url)
      status = url_opened.getcode()
      #Added status check - Lance
      if status != 200:
        return None

      response = url_opened.read()
    ...
    def search(self, query, status, datatype='movies', **kwargs):
        """
        ...
        data = self._load_json_from_url(''.join(search_url), status)
        if data == None:
            return None
    ...
    def lists(self, directory=None, sub=None, **kwargs):
        ...
        data = self._load_json_from_url(''.join(lists_url), status)
        return data

    def info(self, id_num, status, datatype='reviews', specific_info='reviews', **kwargs):
        """
        ...
        kwargs.update({'apikey': self.api_key})
        movie_url.extend(['.json?', urlencode(kwargs)])
